...
const SelectGadget = ({ selectedChart, chartOptions, onSelect, gadgetId }) => {
  ...
  return (
    <div className="relative flex-grow">
        <BiGridVertical size={16} className="text-gray-500 pointer-events-none absolute inset-y-0 left-[7px] my-auto" />
      <select
        value={selectedChart}
...
const DashboardConfig = () => {
  ...
  const handleAddGadget = () => {
    const nextId = gadgets.length > 0 ? Math.max(...gadgets.map(g => g.id)) + 1 : 1;
    setGadgets((prevGadgets) => [
      ...prevGadgets,
    ...
    <div className="border border-gray-300 rounded-xs p-4 my-4">
      <div className="flex justify-between items-center pb-4">
        ...
        <div className="flex space-x-2 rounded-md p-1">
          ...
          <button className="p-1 rounded bg-gray-200 hover:bg-blue-600 text-gray-600 hover:text-white" title="History">
